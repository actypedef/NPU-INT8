...
TransposeStatus GetTransposeStatus(const at::Tensor &mat)
{
    ...
    // A 2D tensor is considered transposed (Column Major) if its strides are (1, dim0).
    if (mat.dim() == 2) {
        if (mat.strides().at(0) == 1 && mat.strides().at(1) == mat.sizes().at(0)) {
            return TransposeStatus::TRANSPOSE;
        }
    }
    // Fallback for other cases or more complex tensors
    return TransposeStatus::NON_CONTINUOUS;
}
...
// New helper function for QuantMatmul
KernelInfo GetQuantKernelInfo(const at::Tensor &matA, const at::Tensor &matB, const at::Tensor &scale,
                                const at::Tensor &perTokenScale)
{
    KernelInfo kernelInfo;
    // Shape checks
    int64_t m = matA.sizes().at(0);
    int64_t k1 = matA.sizes().at(1);
    int64_t k2 = matB.sizes().at(0);
    int64_t n = matB.sizes().at(1);

    if (k1 != k2) {
        throw std::runtime_error("Matrix multiplication dimension mismatch for k.");
    }
    if (scale.sizes().size() != 1 || scale.sizes().at(0) != n) {
        throw std::runtime_error("Shape mismatch for scale tensor.");
    }
    if (perTokenScale.sizes().size() != 1 || perTokenScale.sizes().at(0) != m) {
        throw std::runtime_error("Shape mismatch for perTokenScale tensor.");
    }
    // Type checks
    if (matA.scalar_type() != torch::kInt8 || matB.scalar_type() != torch::kInt8) {
        throw std::runtime_error("Input matrices must be of type int8.");
    }
    if (scale.scalar_type() != torch::kBFloat16 || perTokenScale.scalar_type() != torch::kBFloat16) {
        throw std::runtime_error("Scale tensors must be of type bfloat16.");
    }

    kernelInfo.m = m;
    kernelInfo.k = k1;
    kernelInfo.n = n;
    kernelInfo.inputDataType = ACL_INT8;
    kernelInfo.outputDataType = ACL_BF16;

    // The kernel is implemented for RowMajor A and ColumnMajor B
    if (GetTransposeStatus(matA) != TransposeStatus::NO_TRANSPOSE) {
        throw std::runtime_error("matA must be contiguous (Row Major).");
    }
    if (GetTransposeStatus(matB) != TransposeStatus::TRANSPOSE) {
        throw std::runtime_error("matB must be transposed (Column Major).");
    }
...
// New wrapper function for QuantMatmul
at::Tensor RunQuantMatmul(const at::Tensor &matA, const at::Tensor &matB, const at::Tensor &scale,
                            const at::Tensor &perTokenScale)
{
    KernelInfo kernelInfo = GetQuantKernelInfo(matA, matB, scale, perTokenScale);
    uint32_t aicCoreNum = platform_ascendc::PlatformAscendCManager::GetInstance()->GetCoreNumAic();
    aclrtStream stream = c10_npu::getCurrentNPUStream().stream(false);
    ...
    size_t lenWorkspace = static_cast<size_t>(L1_TILE_M) * L1_TILE_N * aicCoreNum * WORKSPACE_STAGES;
    size_t sizeWorkspace = lenWorkspace * sizeof(uint32_t); // The example uses uint32_t for workspace
    uint8_t *deviceWorkspace{nullptr};
    aclrtMalloc(reinterpret_cast<void **>(&deviceWorkspace), sizeWorkspace, ACL_MEM_MALLOC_HUGE_FIRST);
    kernelInfo.workspaceAddr = deviceWorkspace;

    // Prepare input addresses
    kernelInfo.inputAddr.resize(4);
    kernelInfo.inputAddr[0] = static_cast<uint8_t *>(const_cast<void *>(matA.storage().data()));
    kernelInfo.inputAddr[1] = static_cast<uint8_t *>(const_cast<void *>(matB.storage().data()));
    kernelInfo.inputAddr[2] = static_cast<uint8_t *>(const_cast<void *>(scale.storage().data()));
    kernelInfo.inputAddr[3] = static_cast<uint8_t *>(const_cast<void *>(perTokenScale.storage().data()));

    // Prepare output tensor and address
    torch::Tensor result = GetOutputTensor(InferShape(matA.sizes(), matB.sizes()), torch::kBFloat16);
    kernelInfo.outputAddr.resize(1);
    kernelInfo.outputAddr[0] = static_cast<uint8_t *>(const_cast<void *>(result.storage().data()));
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("run_basic_matmul", &RunBasicMatmul, "basic matmul implemented by catlass");
    m.def("run_grouped_matmul", &RunGroupedMatmul, "grouped matmul implemented by catlass");
    m.def("run_optimized_matmul", &RunOptimizedMatmul, "optimized matmul implemented by catlass");
    m.def("run_quant_matmul", &RunQuantMatmul, "quant matmul (int8) implemented by catlass");
}
